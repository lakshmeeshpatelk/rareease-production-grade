...
export async function POST(req: NextRequest) {
  ...
  const { error: pErr } = await (admin as any).from('products').upsert(product, { onConflict: 'id' });
  if (pErr) return NextResponse.json({ error: pErr.message }, { status: 500 });

  for (const v of (variants ?? [])) await (admin as any).from('variants').upsert(v, { onConflict: 'id' });
  for (const inv of (inventory ?? [])) {
    const { id: _id, ...invData } = inv as Record<string, unknown>;
    await (admin as any).from('inventory').upsert(invData, { onConflict: 'variant_id' });
  }
...
export async function PATCH(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  logAdminAction('product.update', { resource_id: `product:${id}`, meta: update, req });
  return NextResponse.json({ ok: true });
}
...
export async function DELETE(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  logAdminAction('product.delete', { resource_id: `product:${id}`, req });
  return NextResponse.json({ ok: true });
}
