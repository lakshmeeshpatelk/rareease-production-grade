...
export type PhaseType = 'choose' | 'phone-input' | 'phone-otp' | 'email-input' | 'email-otp' | 'signed-in' | 'orders' | 'addresses' | 'reviews' | 'sizeGuide';
...
export default function AccountPanel() {
  ...
  /* ── Email: magic link (legacy — kept for direct sendEmailLink calls) ── */
  const sendEmailLink = async () => {
    ...
    const { error } = await (getClient() as any).auth.signInWithOtp({
      email: email.trim(),
      options: {
        emailRedirectTo: process.env.NEXT_PUBLIC_APP_URL ?? 'https://rareease.com',
        data: {},
      },
    });
    setLoading(false);
  ...
  const signInWithGoogle = async () => {
    ...
    const { error } = await (getClient() as any).auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: process.env.NEXT_PUBLIC_APP_URL ?? 'https://rareease.com' },
    });
  ...
  const renderPhase = () => {
    switch (phase) {
      ...
      case 'phone-otp':   return <AuthPhone
        ...
        />;
      case 'email-input':
      case 'email-otp':   return <AuthEmail
          onBack={() => setPhase('choose')}
          onSuccess={(u) => { setUser(u); setPhase('signed-in'); }}
          addToast={addToast}
        />;
