...
/**
 * AuthEmail — self-contained email OTP login component.
 * Mirrors AuthPhone.tsx exactly: two-step flow (email → 6-digit OTP).
 * Owns its own email/otp/loading state so AccountPanel never re-renders
 * and remounts this input mid-typing.
 *
 * Supabase sends a 6-digit code when signInWithOtp is called WITHOUT
 * emailRedirectTo. Verified via verifyOtp({ email, token, type: 'email' }).
 */
...
/* ── Reusable pieces (identical to AuthPhone) ─────────────────── */
...
function AuthBtn({
  ...
  return (
    <button
      ...
      style={{
        width: '100%', minHeight: 48, background: 'var(--white)', color: 'var(--black)',
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <input
          ...
          style={{
            ...
            border: `1px solid ${value[i] ? 'rgba(255,255,255,0.4)' : 'rgba(255,255,255,0.12)'}`,
            color: 'var(--white)', fontSize: 22, fontFamily: 'var(--font-display)',
...
/* ── Main component ──────────────────────────────────────────────── */
export default function AuthEmail({ onBack, onSuccess, addToast }: Props) {
  ...
  /* Send 6-digit OTP to email (no emailRedirectTo = OTP code, not magic link) */
  const sendOtp = async () => {
  ...
  if (step === 'email') {
    return (
      <div style={{ padding: '32px 24px', display: 'flex', flexDirection: 'column' }}>
        <button
          onClick={onBack}
          style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.35)', fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', cursor: 'pointer', textAlign: 'left', marginBottom: 28, display: 'flex', alignItems: 'center', gap: 6 }}
        >
          ← Back
        </button>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)' }}>
            {/* Email icon prefix */}
            <span style={{ padding: '0 12px', borderRight: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', color: 'rgba(255,255,255,0.35)' }}>
  ...
  return (
    <div style={{ padding: '32px 24px', display: 'flex', flexDirection: 'column' }}>
      ...
      <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', lineHeight: 1.6, margin: '0 0 28px' }}>
        OTP sent to <strong style={{ color: 'rgba(255,255,255,0.6)' }}>{email}</strong>
      </p>
      ...
      {otpError && (
        <p style={{ fontSize: 11, color: '#ff6b6b', textAlign: 'center', marginBottom: 16 }}>
          {otpError}
        </p>
      )}
