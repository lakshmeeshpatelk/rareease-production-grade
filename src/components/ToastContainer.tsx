...
export default function TrendingSection() {
  ...
  // ── Native touch handlers (passive:false so we can preventDefault on horizontal drags)
  // React 17+ makes synthetic touch events passive — e.preventDefault() is silently ignored.
  // Native DOM listeners with { passive: false } are required to prevent the PAGE from
  // scrolling sideways while the user swipes the carousel on iOS Safari.
  useEffect(() => {
    const el = scrollRef.current;
    if (!el) return;
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      if (Math.abs(dx) > dy && Math.abs(dx) > 4) {
        e.preventDefault(); // Prevent iOS page scroll during horizontal carousel swipe
    ...
    const handleTouchEnd = () => {
      // Longer delay to let iOS momentum scrolling finish before re-enabling auto-scroll
  ...
  return (
    <section className="trend-section" id="trending">
      ...
      {/* ── Infinite carousel — touch handled by native listeners in useEffect above ── */}
      <div
        ref={scrollRef}
